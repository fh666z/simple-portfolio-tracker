...
The script will:
- Build a one-file executable (`PortfolioTracker.exe`) using PyInstaller
- Include the application icon from `assets/icon.ico`
- Bundle the `core/` and `ui/` directories as data files
- Include hidden imports: PyQt6.sip, openpyxl, PIL
- Clean previous build artifacts before building
- Create a windowed executable (no console window)

**Output location:**
- `dist/PortfolioTracker.exe` - Single standalone executable

### Build Configuration

The build script (`build.py`) uses PyInstaller with the following configuration:
- **Mode**: One-file executable (single file distribution)
- **Icon**: `assets/icon.ico` (application icon)
- **Windowed**: No console window (GUI application)
- **Data files**: `core/` and `ui/` directories are bundled
- **Hidden imports**: PyQt6.sip, openpyxl, PIL

### Application Icon

The app uses `assets/icon.ico` for the executable icon. If you want to customize it:
- Replace `assets/icon.ico` with your own `.ico` file
- Recommended sizes: 16×16, 32×32, 48×48, 256×256
- Rebuild the executable after replacing the icon

### Notes

- **Tesseract OCR**: Not bundled with the executable. Users must install Tesseract separately for image import functionality. See the Prerequisites section above.
- **Windows Defender/Antivirus**: May flag the executable on first run. This is common with PyInstaller executables. You may need to add an exception or sign the executable.
- **File Size**: The one-file executable will be large (typically 50-100MB) as it includes Python, PyQt6, matplotlib, and all dependencies.
